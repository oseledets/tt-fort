...
contains
 subroutine ort0_d(inp,out,mat)
  implicit none
  double precision,intent(inout) :: inp(:,:)
  double precision,intent(out),optional :: out(size(inp,1),size(inp,2))
...
!-
 ...
 subroutine ort1_d(u,v,y,b)
  ...
  double precision,intent(out),optional :: y(size(v))
  double precision,intent(out),optional :: b(size(u,2)+1)
  character(len=*),parameter :: subnam='ort1_d'
  integer :: nu,nv,n,r,pass
  double precision,allocatable :: g(:),btmp(:)
 ...
 subroutine orto_d(u,v,out,mat)
  ...
  double precision,intent(in) :: u(:,:)
  double precision,intent(inout) :: v(:,:)
  double precision,intent(out),optional :: out(size(v,1),size(v,2))
  double precision,intent(out),optional :: mat(size(u,2)+size(v,2),size(v,2))
  character(len=*),parameter :: subnam='orto_d'
  integer :: ru,rv,nu,nv,n,j, r,pass
  double precision,allocatable :: x(:),gu(:),gv(:),btmp(:,:)
  double precision,pointer :: bb(:,:),yy(:,:)
  double precision :: nrm1,nrm2
  logical :: reort
  double precision,external :: dnrm2
  target :: v,out,mat,btmp
