...
!----
 ...
 pure double precision function dfun(ind,par) result (f)
  use pot_lib
  use bt_lib
  implicit none
  type(ttind),intent(in) :: ind
  double precision,intent(in),optional :: par(:)
  double precision,parameter :: tpi=6.28318530717958647692528676655900577d0, d2r=tpi/360
  double precision :: x,y,z,r,hh,h
  type(ttind) :: ii,jj,kk
  integer :: dd,id,i,j!,ijacobi=0
  double precision :: ppp(2,128),p(128),q(128)
  double precision,allocatable :: mat(:,:)
  !data ppp /1.5d0,2.d0, 1.50,2.d0, 2.5d0,3.d0, 90.d0,110.d0, 70.d0,90.d0, 100.d0,130.d0 /
  !double precision,external :: hoohpot_func

  if(.not.all(ind%p(1:ind%m)>0))then;f=-1.d0;return;endif
  if(.not.all(ind%p(1:ind%m)<=ind%n(1:ind%m)))then;f=-2.d0;return;endif
  

!  r=1.d0
!  do id=1,ind%m
!   !r=r+(2*dble(ind%p(id))-ind%n(id)-1)/( (ind%n(id)-1) * ((1.d0+id)**4) )
!   r=r+(2*dble(ind%p(id))-ind%n(id)-1)/( (ind%n(id)-1) * dexp((1.d0+id)*3) )
!  end do
!  f=1.d0/r
!  return

!  !fin :: i123d j123d ... k123d
!  dd=6
!  allocate(mat(dd,dd)); forall(i=1:dd,j=1:dd)mat(i,j)=dble(min(i,j))
!  if(mod(ind%m,dd).ne.0)then;f=-1.d0;return;endif
!  do id=1,dd
!   ii%m=ind%m/dd; ii%n(1:ii%m)=ind%n(id:ind%m:dd); ii%p(1:ii%m)=ind%p(id:ind%m:dd)
!   !ii%m=ind%m/dd; ii%n(1:ii%m)=ind%n(1+(id-1)*ii%m:id*ii%m); ii%p(1:ii%m)=ind%p(1+(id-1)*ii%m:id*ii%m)
!   p(id)=(dble(ii)-.5d0)/(2.d0**ii%m)
!   !write(*,'(10i1,1x,f10.6)')ii%p(1:ii%m),p(id)
!  end do
!  q(1:dd)=matmul(mat,p(1:dd))
!  r=0.d0; do id=1,dd; r=r+q(id)*p(id); enddo
!  f=dexp(-20.d0*r) * maxval(p(1:dd))!*dexp(-20.d0*r)
!  return
  

  ! dbt ttoeplitz
  f=dbt_ttoep2(dble(ind))
  return
  ...
  do id=1,dd
   !ii%m=ind%m/dd; ii%n(1:ii%m)=ind%n(id:ind%m:dd); ii%p(1:ii%m)=ind%p(id:ind%m:dd)
   ii%m=ind%m/dd; ii%n(1:ii%m)=ind%n(1+(id-1)*ii%m:id*ii%m); ii%p(1:ii%m)=ind%p(1+(id-1)*ii%m:id*ii%m)
   p(id)=par(2*id-1) + (dble(ii)-0.5d0)*(par(2*id)-par(2*id-1))/(2.d0**ii%m)
   !if(.not. ( par(2*id-1).lt.p(id) .and. p(id).lt.par(2*id)))stop
  end do
  !f=dbt(p(1),p(2),p(3),p(4),p(5))
  f=dbt(.5d0,.5d0,0.d0,0.d0,30.d0,p(1),p(2),p(3),p(4))

  !hootpot :: rrraaa1 rrraaa2 rrraaa3 ... rrraaad
  !dd=6
  !do id=1,dd
  ! ii%m=ind%m/dd; ii%n(1:ii%m)=ind%n(id:ind%m:dd); ii%p(1:ii%m)=ind%p(id:ind%m:dd)
  ! p(id)=ppp(1,id) + (dble(ii)-0.5d0)*(ppp(2,id)-ppp(1,id))/(2.d0**ii%m)
  ! if(.not. ( ppp(1,id).lt.p(id) .and. p(id).lt.ppp(2,id)))stop
  !end do
  !f=hoohpot_func(ijacobi,p(1),p(2),p(3),p(4)*d2r,p(5)*d2r,p(6)*d2r)
  
  !! 3D 1/r potential i1..id j1..jd k1..kd
  !dd=3; r=0.d0; h=par(1)
  !do id=1,dd
  ! ii%m=ind%m/dd; ii%n(1:ii%m)=ind%n(1+(id-1)*ind%m/dd:id*ind%m/dd); ii%p(1:ii%m)=ind%p(1+(id-1)*ind%m/dd:id*ind%m/dd)
  ! x=dble(ii)-1.d0; x=dmin1(x, (2.d0**ii%m)-x)
  ! p(id)=x*h
  !end do
  !f=pot3rinv(p(1)-h/2,p(1)+h/2,p(2)-h/2,p(2)+h/2,p(3)-h/2,p(3)+h/2,0.d0,0.d0,0.d0)/(h**3)
  !return
  
  !f=0.d0
  !up=1.d16; h=up/(2.d0**d)
  !f=1.d0/sqrt(ind*h-h/2) 
  
  !! sine2
  !!x=tpi*h*(dble(ind)-.5d0)
  !!f=.25d0*dsin(x)+.75d0*dsin(7.d0*x)
  
  !! sinc
  !x=(dble(ind)-1.d0)
  !if(2.d0*x.ge.2.d0**ind%m)x=x-2.d0**ind%m
  !if(dabs(x).gt.1.d-12)then
  ! x=x*h*tpi/2
  ! f=(dsin(x)/x)**2
  !else
  ! f=1.d0
  ...
  ! slater2 ijk1 ijk2 ijk3 ... ijkd
  !ii%m=ind%m/2; ii%n(1:ii%m)=ind%n(1:ind%m:2); ii%p(1:ii%m)=ind%p(1:ind%m:2)
  !jj%m=ind%m/2; jj%n(1:jj%m)=ind%n(2:ind%m:2); jj%p(1:jj%m)=ind%p(2:ind%m:2)
  !x=h*(dble(ii)-.5d0); y=h*(dble(jj)-.5d0)
  !r=dsqrt(x**2+y**2)
  !f=exp(-r)/r
  
  ! slaterD ijk1 ijk2 ijk3 ... ijkd
  !dd=3; r=0.d0
  !do id=1,dd
  ! ii%m=ind%m/dd; ii%n(1:ii%m)=ind%n(id:ind%m:dd); ii%p(1:ii%m)=ind%p(id:ind%m:dd)
  ! x=h*(dble(ii)-.5d0); r=r+x**2
  !end do
  !r=dsqrt(r); f=exp(-r)/r
  
  ! slaterD i1..id j1..jd k1..kd
  !dd=4; r=0.d0
  !do id=1,dd
  ! ii%m=ind%m/dd; ii%n(1:ii%m)=ind%n(1+(id-1)*ind%m/dd:id*ind%m/dd); ii%p(1:ii%m)=ind%p(1+(id-1)*ind%m/dd:id*ind%m/dd)
  ! x=h*(dble(ii)-.5d0); r=r+x**2
  ...
  ! slater2 i1..id j1..jd k1..kd
  !!ii%m=ind%m/2; ii%n(1:ii%m)=ind%n(1:ind%m/2);        ii%p(1:ii%m)=ind%p(1:ind%m/2)
  !!jj%m=ind%m/2; jj%n(1:jj%m)=ind%n(1+ind%m/2:ind%m);  jj%p(1:jj%m)=ind%p(1+ind%m/2:ind%m)
  !!x=h*(dble(ii)-.5d0); y=h*(dble(jj)-.5d0)
  !!r=dsqrt(x**2+y**2)
  !!f=exp(-r)/r
  return
 end function
 pure double complex function zfun(ind,par) result (f)
  use pot_lib
  implicit none
  type(ttind),intent(in) :: ind
